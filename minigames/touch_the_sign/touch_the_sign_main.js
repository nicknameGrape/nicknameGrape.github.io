...
require(["fitText", "fitTextArray", "HatDraw", "touch_the_sign/signs", "Loader", "fitImage"], function (fitText, fitTextArray, HatDraw, hdSigns, Loader, fitImage) {
	function start() {
	...
	function loop() {
		...
		if (mistakes < 3) {
			...
			xMap.clearRect(0, 0, xMap.canvas.width, xMap.canvas.height);
			let width = sign.x2-sign.x1; let height = sign.y2-sign.y1;
			//xMap.drawImage(img, sign.x1-width*2, sign.y1-height*2, width*5, height*5, 0, 0, xMap.canvas.width, xMap.canvas.height);
			fitImage(xMap, img, sign.x1-width*2, sign.y1-height*2, width*5, height*5, 0, 0, xMap.canvas.width, xMap.canvas.height);
	...
	var img = loader.newImageAsset("lets_try_2_abc_hunt.jpg", onload);
